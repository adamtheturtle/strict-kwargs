...
# `vendored/typeshed/{stdlib,LICENSE}`, records the resolved commit in
# `vendored/typeshed/COMMIT`, then builds and tests so a bad sync fails loudly.
...
Next steps:
  - Review the diff:   git -C "${ROOT}" status --short vendored/typeshed
  - Commit:            git add vendored/typeshed && git commit -m "Bump vendored typeshed to ${COMMIT:0:12}"
