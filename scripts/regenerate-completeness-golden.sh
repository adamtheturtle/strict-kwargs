...
COMPLETENESS_TY_VERSION="${STRICT_KWARGS_COMPLETENESS_TY_VERSION:-0.0.64}"
...
# Snapshot selection follows the host platform. The linux oracle must be
# regenerated on linux and the macOS oracle on macOS: this script records
# the diagnostics the *host* produces, so writing one platform's results
# into the other platform's snapshot silently destroys that platform's
# floor.
