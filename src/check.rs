...
impl<'a> CallChecker<'a> {
    ...
    fn check_call(&mut self, call: &ast::ExprCall) {
        ...
        let local_function = if let Some(signature) =
            self.update_wrapper_result_signature(&call.func)
        {
            Some(LocalFunction {
    ...
    fn identity_return_callable(&self, func: &Expr) -> Option<String> {
        ...
        // ``recursive_repr(...)(target)`` / ``lru_cache()(user_function=target)`` /
        // ``wraps(wrapped=target)(wrapper=target)``.
    ...
    #[cfg_attr(coverage, coverage(off))]
    fn dict_values_item_signature(&self, expr: &Expr) -> Option<Signature> {
